...
cdef class VL53L5CX:
    ...
    def __cinit__(self, dev_path: str, target_addr: np.uint8_t, freq: np.uint8_t, transform: np.uint8_t) -> None:
        self.transform = transform
        cdef int32_t status = vl53l5cx_py_init(&self.dev_conf, dev_path, target_addr, freq)
        if status != 0:
            raise RuntimeError
